...
// === FORCE HOME POSITION FROM NOTETAG (by Elise) ===
Sprite_Actor.prototype.setActorHome = function(index) {
  // 1) Coba baca dari notetag Actor/Class
  ...
  function readMetaXY(obj) {
    if (!obj || !obj.meta) return null;
    // Prioritas: <Home Position: x, y>
    if (obj.meta['Home Position']) return parseXY(obj.meta['Home Position']);
    // Alternatif: <Sideview Battler X: n> + <Sideview Battler Y: n>
    var ax = obj.meta['Sideview Battler X'];
    var ay = obj.meta['Sideview Battler Y'];
  ...
  if (actor) {
    // Cek Actor dulu
    data = readMetaXY(actor.actor());
    // Kalau kosong, cek Class
    if (!data) data = readMetaXY(actor.currentClass());
  }

  if (data) {
    // 2) Kalau ada di notetag → PAKSA pakai itu
    this._homeX = data.x;
    this._homeY = data.y;
    this.setHome(this._homeX, this._homeY);
  } else {
    // 3) Fallback ke perilaku BEC lama (biar gak rusak)
    // (kebanyakan versi BEC pakai BattleManager.actorHomeX/Y atau formula param)
    var x = (BattleManager.actorHomeX) ? BattleManager.actorHomeX(this) : this.x;
    var y = (BattleManager.actorHomeY) ? BattleManager.actorHomeY(this) : this.y;
    this._homeX = x;
    this._homeY = y;
    this.setHome(x, y);
  }
...
// === FORCE VERTICAL STEP (no X drift) ===
Sprite_Actor.prototype.stepForward = function() {
  var d = Yanfly.Param.BECStepDist || 24;   // jarak maju (px)
  var s = Yanfly.Param.BECStepDur  || 12;   // durasi (frame)
  this.startMotion('walk');                 // atau 'thrust' kalau mau
  this.startMove(0, -d, s);                 // MAJU = naik (Y negatif)
};
...
Sprite_Actor.prototype.stepBack = function() {
  var d = Yanfly.Param.BECStepDist || 24;
  var s = Yanfly.Param.BECStepDur  || 12;
  this.startMotion('wait');
  this.startMove(0, d, s);                  // MUNDUR = turun (Y positif)
};
...
// --- OVERRIDE PALING BAWAH FILE ---
Sprite_Actor.prototype.stepForward = function(){ var d=24,s=12; this.startMotion('walk'); this.startMove(0,-d,s); };
Sprite_Actor.prototype.stepBack    = function(){ var d=24,s=12; this.startMotion('wait'); this.startMove(0, d,s); };

// === NO FLINCH AT ALL ===
Sprite_Actor.prototype.stepFlinch = function() { /* no-op */ };
Sprite_Enemy.prototype.stepFlinch = function() { /* no-op */ };

// === Rear-View Clean Step (by Elise) ===
// - Tidak maju saat input/awal battle.
// - Maju hanya untuk Attack & skill yang benar-benar menyerang (HP/MP damage).
// - Tidak maju untuk Guard & Item.
// - Mundur kembali setelah aksi selesai.
// Catatan: stepForward/stepBack vertikal kamu tetap dipakai (0,-d, s) dan (0, d, s).

(function(){
  'use strict';

  // Gate: cegah panggilan stepForward "liar" (mis. dari ActSeqPack saat input)
  const _rv_origStepForward = Sprite_Actor.prototype.stepForward;
  Sprite_Actor.prototype.stepForward = function(){
    // Hanya izinkan maju kalau memang diizinkan patch ini
    if (this._rv_allowAdvance) {
      this._rv_allowAdvance = false; // pakai sekali
      _rv_origStepForward.call(this);
    } else {
      // selain itu: abaikan (agar tidak maju saat input/awal battle)
      return;
    }
  };

  // Ambil Sprite_Actor dari Game_Actor
  function spriteForActor(gameActor){
    const sc = SceneManager._scene;
    if (!sc || !sc._spriteset) return null;
    const arr = sc._spriteset._actorSprites || [];
    for (let i=0;i<arr.length;i++){
      if (arr[i] && arr[i]._battler === gameActor) return arr[i];
    }
    return null;
  }

  // Helper: apakah action ini harus maju?
  function shouldAdvanceFor(action){
    if (!action) return false;
    // Jangan maju untuk Guard
    if (action.isGuard && action.isGuard()) return false;
    // Jangan maju untuk Item (apapun item-nya)
    if (action.isItem && action.isItem()) return false;

    // Attack default → maju
    if (action.isAttack && action.isAttack()) return true;

    // Skill: maju hanya jika benar-benar memberi damage HP/MP (type 1 atau 2)
    if (action.isSkill && action.isSkill()) {
      try {
        const dmgType = action.item().damage.type; // 0:none,1:HP dmg,2:MP dmg,3:HP rec,4:MP rec,5:HP drain,6:MP drain
        if (dmgType === 1 || dmgType === 2 || dmgType === 5 || dmgType === 6) return true;
      } catch(e) {}
    }
    return false;
  }

  // Saat AKTOR benar-benar mulai mengeksekusi aksi (bukan saat input)
  const _Game_Actor_performAction = Game_Actor.prototype.performAction;
  Game_Actor.prototype.performAction = function(action){
    _Game_Actor_performAction.call(this, action);
    const spr = spriteForActor(this);
    if (!spr) return;

    // Reset flag dulu
    spr._rv_allowAdvance = false;
    spr._rv_advanced = false;

    if (shouldAdvanceFor(action)) {
      // Izinkan maju lalu panggil stepForward (vertikal versi kamu)
      spr._rv_allowAdvance = true;
      spr.stepForward();
      spr._rv_advanced = true;
    }
  };

  // Setelah aksi selesai → kalau tadi maju, mundur balik ke home
  const _BattleManager_endAction = BattleManager.endAction;
  BattleManager.endAction = function(){
    const subj = this._subject;
    if (subj && subj.isActor && subj.isActor()){
      const spr = spriteForActor(subj);
      if (spr && spr._rv_advanced){
        spr._rv_advanced = false;
        spr._rv_allowAdvance = false;
        spr.stepBack(); // vertikal versi kamu
      }
    }
